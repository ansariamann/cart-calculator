...
export const CameraScanner = ({
          ...
          async (decodedText) => {
            ...
            try {
              ...
              if (data.status === 1 && data.product) {
                ...
                onItemScanned({
                  name:
                    product.product_name || `Product ${decodedText.slice(-6)}`,
                  price: 1, // Default price
                  quantity: 1,
                  category:
                    product.categories?.split(",")[0]?.trim() || "Scanned",
                });
  ...
  return createPortal(
    ...
    >
      ...
      <p className="mt-6 text-white/70 text-sm">Point camera at a barcode</p>
    </div>,
    document.body
  );
};
