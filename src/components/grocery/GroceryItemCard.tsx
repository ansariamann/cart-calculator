...
interface GroceryItemCardProps {
  ...
  onAddToFavorites?: (item: { name: string; price: number; category?: string }) => void;
  isFavorite?: boolean;
  index: number;
}
...
export const GroceryItemCard = ({ 
  ...
  const { click, remove, success } = useSoundEffects();
  ...
  const handleLongPressStart = () => {
    longPressTimer.current = setTimeout(() => {
      if (onAddToFavorites && !isFavorite) {
        success();
        onAddToFavorites({ name: item.name, price: item.price, category: item.category });
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {item.checked && <Check className="w-3.5 h-3.5 text-primary-foreground" strokeWidth={3} />}
          </button>
