...
export const generateReceiptPDF = (
  ...
  items.forEach((item) => {
    ...
    doc.text(`Rs. ${itemTotal.toFixed(2)}`, pageWidth - 5, y, { align: "right" });
    y += 4;
  });
  ...
  doc.text(`Rs. ${calculation.subtotal.toFixed(2)}`, pageWidth - 5, y, {
    align: "right",
  });
  y += 4;
  ...
  if (calculation.discountAmount > 0) {
    ...
    doc.text(`-Rs. ${calculation.discountAmount.toFixed(2)}`, pageWidth - 5, y, {
      align: "right",
    });
    y += 4;
  }
  ...
  doc.text(`Rs. ${calculation.taxAmount.toFixed(2)}`, pageWidth - 5, y, {
    align: "right",
  });
  y += 4;
  ...
  doc.setFont("helvetica", "bold");
  doc.text("TOTAL:", 5, y);
  doc.text(`Rs. ${calculation.total.toFixed(2)}`, pageWidth - 5, y, {
    align: "right",
  });
  y += 8;
